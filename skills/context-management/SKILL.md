...
Inline signals throughout this skill reference `context=` / `tool=` / `since-anchor=` from the pi-control status line.
...
`tree(compact)` summarizes older entries on the current path to free up context. It keeps direction; use `pivot` when you want to change direction instead.

pi auto-compacts near the context window limit. Trigger manually when you want to drop tool noise earlier or focus the summary with `customInstructions`.
